    ...
    if (parsedData.op == OPCODES.HELLO) {
        ...
        lanyard.send(
            JSON.stringify({
                op: OPCODES.INIT,
                d: {
                    subscribe_to_id: "643473809663459357",
                },
            })
        );
        ...
        if (parsedData.t == "INIT_STATE") {
            ...
            elements.avatar.src = `https://cdn.discordapp.com/avatars/643473809663459357/${user.discord_user.avatar}?size=128`;
            var a = 0
            console.log(user)
            ...
            elements.durum.innerText ="Discord Durumu: Anything"
            } else if ( a == 2) {
            elements.durum.innerText ="Discord Durumu: "+ user.activities[0].state
            } else if (a == 0){
            elements.durum.innerText ="Discord Durumu: "+ user.activities[0].state+"\n Şuan Ne yapıyorum: "+user.activities[1].name
